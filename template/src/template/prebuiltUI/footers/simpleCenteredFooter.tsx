import { Box, BoxProps, Flex, FlexProps, Icon, Spacer, useColorModeValue, Wrap, WrapItem, WrapProps } from '@chakra-ui/react'
...
export const SimpleCenteredFooter = (props: SimpleCenteredFooterProps) => {
  ...
  return (
    <Flex {...overrideStyles(defaultFlexContainerStyles, styles?.flexContainerProps)}>
      <Box>
        <Wrap {...overrideStyles(defaultPageLinksWrapStyles, styles?.pageLinkWrapProps)}>
          {pageLinks.map((pageLink => (
            <WrapItem key={`footer-${pageLink.href}`}>
              <Link href={ pageLink.href } passHref>
                <Box as='a' {...overrideStyles(defaultPageLinkStyles, styles?.pageLinkProps)}>
                  { pageLink.text }
                </Box>
              </Link>
            </WrapItem>
