...
export const BasicHeader = (props: BasicHeaderProps) => {
  const { isAuthed, companyIcon, loginButton, links, styles } = props;
  ...
  if (isAuthed) {
    finalLinks = links.concat([{ text: "App", href: "/app", type: "link" }]);
  }
            ...
            return (
              <Link href={link.href} passHref key={link.text}>
                <Box
                  as="a"
                  {...overrideStyles(
                    defaultHeaderLinkStyles,
                    styles?.headerLinkProps
                  )}
                >
                  {link.text}
                </Box>
              ...
              <Menu key={link.title}>
                <MenuButton
                  {...overrideStyles(
                    ...
                    styles?.dropDownMenuButtonProps
                  )}
                >
                  <Box
                    {...overrideStyles(
                ...
                <MenuList
                  {...overrideStyles(
                    defaultDropDownListStyles,
                    styles?.dropDownMenuListProps
                  )}
                >
                  {link.links.map(
                    (dropDownLink: { text: string; href: string }) => (
                      <MenuItem key={dropDownLink.text}>
                        <Link href={dropDownLink.href} passHref>
                          <Box>{dropDownLink.text}</Box>
                        </Link>
                      </MenuItem>
                    )
                  )}
